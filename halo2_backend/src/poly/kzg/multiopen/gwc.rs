...
#[derive(Clone, Debug)]
struct CommitmentDataZCash<F, T: PartialEq> {
    commitment: T,
    set_index: usize,
...
impl<F, T: PartialEq> CommitmentDataZCash<F, T> {
...
type IntermediateSets<F, Q> = (
    Vec<CommitmentDataZCash<<Q as Query<F>>::Eval, <Q as Query<F>>::Commitment>>,
    Vec<Vec<F>>,
);

fn construct_intermediate_sets_zcash<F: Field + Ord, I, Q: Query<F>>(
    queries: I,
    ...
    I: IntoIterator<Item = Q> + Clone,
{
    ...
    // their queries.
    let mut commitment_map: Vec<CommitmentDataZCash<Q::Eval, Q::Commitment>> = vec![];
    ...
    for query in queries.clone() {
            ...
            .position(|comm| comm.commitment == query.get_commitment())
        {
            ...
            let mut tmp = CommitmentDataZCash::new(query.get_commitment());
